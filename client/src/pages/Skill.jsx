...
const skills = [
  ...
  },
  {
    icon: <SiExpress />,
    name: "Express",
    desc: "Building RESTful APIs with modular architecture.",
  },
];
...
const InteractiveStack = () => {
  ...
  return (
    <section className="min-h-screen bg-[#0f1116] text-gray-200 px-6 py-20">
      <h2 className="text-4xl font-bold text-center mb-14 text-white">
        Interactive Stack
      </h2>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-10 max-w-6xl mx-auto">
        {skills.map((skill, i) => (
          <div
            ...
            className="relative group flex flex-col items-center justify-center p-6 rounded-xl bg-[#1a1d24] shadow-md hover:shadow-xl transition cursor-pointer"
          >
            <div className="text-6xl text-gray-400 group-hover:text-cyan-400 transition">
              {skill.icon}
            </div>
            ...
            <div className="absolute bottom-full mb-3 hidden md:group-hover:flex">
              <Tooltip text={skill.desc} />
            </div>

            {/* 🔹 MOBILE TOOLTIP (tap only) */}
            {active === i && (
              <div className="absolute bottom-full mb-3 flex md:hidden">
                <Tooltip text={skill.desc} />
              </div>
            )}
          </div>
